...
const Pricing = () => {
  ...
  return <section className='section'>
    <div className="container mx-auto">
      ...
      > {title} </h2>
      {/* cards */}
      <div className='flex flex-col lg:flex-row lg:gap-x-[30px] gap-y-[30px]
      lg:gap-y-0 justify-center items-center'> 
        {cards.map((card, cardIndex)=> {
        //destructure card
        const {icon, title, services, price, 
          userAmount, btnText, delay} = card
          //card
        return <div
        data-aos='fade-up' 
        ...
        key={cardIndex}>
          ...
          w-[350px] h-[500px]
          rounded-[12px] p-[40px] cursor-pointer transition-all 
        `}>
            {/* card icon */}
            <div className='mb-8'>
              <img src={icon} alt="" />
            ...
            <button className={`${cardIndex === index ? 
              ...
              'border border-accent text-accent'} btn btn-sm space-x-[14px]`}>
              <span>{btnText}</span>
